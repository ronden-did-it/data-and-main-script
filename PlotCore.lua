...
game.Players.PlayerAdded:Connect(function(player)
	...
	while player ~= nil do -- Main game loop
		...
		if loopIndex == 5 then -- Dropping star drops
			loopIndex = 0
			availableDrops[player.Name] = availableDrops[player.Name] + functionality.Stars:GetAttribute("tick") -- Adding to the available amount of drops
	...
	if table.find(loaded, player.Name) then
		...
		if player.UserId > 0 then
			spawn(function()
				...
				pcall(function()
					dustLeader:SetAsync(id, dust)
				end)
				pcall(function()
					starsLeader:SetAsync(id, stars)
				end)
			end)
		end -- Adding player to the leaderboard data store
...
replicatedPlot.Button.OnServerEvent:Connect(function(player, button, value) -- When player steps on a button
	if table.find(uniDelay, player.Name) ~= nil then return end
	...
	local data = dataStore.getDataTable(player)
	if button == "Star" then -- Buying a new star
	...
	elseif button == "Merge" and table.find(mergeDelay, player.Name) == nil then -- Merging the currently owned stars
		...
		return
	elseif button == "Rate" then -- Increasing the rate at which star dust is turned into coins
